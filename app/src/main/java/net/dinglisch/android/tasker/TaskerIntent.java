...
public class TaskerIntent extends Intent {
    ...
    // NoReceiver: Tasker has not created a listener for external access
    // (probably a Tasker bug)
    // OK: you should be able to send a task to run. Still need to listen for
    // result
    // for e.g. task not found

    public static enum Status {
        NotInstalled, NoPermission, NotEnabled, AccessBlocked, NoReceiver, OK
    };
    ...
    // Get an intent that will bring up the Tasker prefs screen with the
    // External Access control(s)
    ...
    // so that if multiple TaskerIntents are used in PendingIntents there's
    // virtually no
    // clash chance
